...
export function clearProcessTracker() {
...
export function trackProcess(transactionId: string, callback: () => void) {
  ...
  const pollingPromise = createAsyncPolling({
    asyncFn: () => api.get(ENDPOINTS.processPendig(transactionId)),
    interval: 3000,
    shouldStop: response => {
