...
function convertToSVGProps(props: Partial<IIConProps>) {
  const { size, width, height, color, ...restProps } = props;
  return {
    ...restProps,
...
function getColor(color: string) {
  const { palette } = rusalTheme;
  return palette?.[color] || color || 'inherit';
}
...
export const RiskPersonalIcon = customizeHOC(Icons.RiskPersonalIcon);
export const RiskBankruptcyIcon = customizeHOC(Icons.RiskBankruptcyIcon);
export const RiskFinishWorkIcon = customizeHOC(Icons.RiskFinishWorkIcon);
export const RiskPayoutMoneyIcon = customizeHOC(Icons.RiskPayoutMoneyIcon);
