...
import { IDbConnection, PageContent, IGuideDTO } from '@mapbul-pub/types';
...
export class GuidesService extends BaseService<IGuideDTO> {
  ...
  async getAll(query: GetAllQueryDTO): Promise<PageContent<IGuideDTO>> {
    let additional = '';
    const isPagination = query.page && query.size;
    if (isPagination) {
      const offset = (query.page - 1) * query.size;
      additional = `limit ${offset},${query.size}; SELECT count(*) FROM guide`;
    }
