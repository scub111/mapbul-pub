...
import { JournalistDTO } from './journalists.dto';
...
export class JournalistsController implements IController<IJournalistDTO> {
  ...
  @UseInterceptors(NotFoundInterceptor)
  async postItem(@Body() body: JournalistDTO): Promise<IJournalistDTO> {
  ...
  @UseGuards(JwtAuthGuard)
  @UseInterceptors(NotFoundInterceptor)
  async putItem(@Param('id') id: string, @Body() body: JournalistDTO): Promise<IJournalistDTO> {
